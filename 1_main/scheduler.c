...
int null_func(SCHED* sched) {
   return  sched->now+1;
}

void sched_marker(SCHED* self) {
   sched_insert(&(self->st),&(self->en), self, null_func, self->now+1 ,1);
}

// https://codeforwin.org/2017/12/pass-function-pointer-as-parameter-another-function-c.html
void sched_reg(SCHED* self, void *typex, int (func_ptr()), int then){
